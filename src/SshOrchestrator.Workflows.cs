            ...
            string profileKey, AppSettings settings, string logDir, CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(hp.Error) && !sawDone)
                {
                    ...
                    hp.Error = timedOut
                        ? "проверка изменений не завершилась (таймаут DNF — вероятно, зеркало недоступно или отвечает медленно)"
                        : "проверка изменений оборвалась без итогового маркера — результат недостоверен";
                }
            }
